"""Grad-CAM heatmaps over the last convolutional block of VGG-16.

Used as the explainable-AI (XAI) overlay on the clinical dashboard.
Falls back to an OpenCV edge/intensity heatmap when TensorFlow is not
available so the web application still produces a visual explanation.
"""
...
def _overlay_heatmap(bgr: np.ndarray, cam: np.ndarray, alpha: float = 0.45) -> np.ndarray:
...
def opencv_saliency_heatmap(image_bgr: np.ndarray) -> np.ndarray:
    """Intensity + edge saliency used when a CNN is not loaded."""
    ...
    return _overlay_heatmap(image_bgr, cam)
...
def generate_gradcam(
    ...
    """Return a BGR overlay highlighting likely fracture regions."""
    ...
    try:
        ...
        cam = tf.reduce_sum(tf.multiply(pooled, conv_outputs), axis=-1)
